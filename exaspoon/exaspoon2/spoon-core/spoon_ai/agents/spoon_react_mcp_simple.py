...
class SpoonReactMCP(SpoonReactAI):
    ...
    def __init__(self, tools=None, persona_name=None, **kwargs):
        ...
        # Proper Pydantic initialization
        super().__init__(**kwargs)

        logger.info(f"Initialized SpoonReactMCP agent: {self.name} with persona: {self.persona_name}")

    async def initialize(self, __context: Any = None):
        """Initialize async components for MCP agent"""
        logger.info(f"Initializing SpoonReactMCP agent '{self.name}'")

        # Ensure x402 tools are available
        try:
